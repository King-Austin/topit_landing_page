    ...
    setTimeout(() => {
      ...
      try {
        confetti({
          ...
          spread: 60,
          origin: { y: 0.6 },
          colors: ['#01875f', '#8ab4f8', '#ffd600', '#00e676']
        });
      } catch {
        // Confetti fallback
      }
    ...
    <div className="my-3 space-y-3">
      ...
      <button
        ...
        className={`w-full py-3 sm:py-3.5 px-6 rounded-full font-medium text-sm sm:text-base flex items-center justify-center space-x-2 transition-all shadow-lg select-none relative overflow-hidden active:scale-[0.98] ${
          downloaded
            ? 'bg-[#1b3a2a] text-[#81c995] border border-[#81c995]/30'
      ...
      >
        ...
        {downloading && (
          <div
            className="absolute left-0 top-0 bottom-0 bg-[#01875f] transition-all duration-200 opacity-30"
        ...
        <div className="relative z-10 flex items-center space-x-2">
          {downloading ? (
            ...
            <>
              <CheckCircle2 className="w-5 h-5 text-[#81c995]" />
